...
namespace CODE.Framework.Wpf.Mvvm
{
    ...
    public class ViewActionRibbon : TabControl
    {
        ...
        private static void SelectedViewChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (actionsContainer != null)
            {
                ...
                viewResult.PropertyChanged -= ribbon.HandleViewResultPropertyChanged;
                viewResult.PropertyChanged += ribbon.HandleViewResultPropertyChanged;
            }
            else
                ribbon.PopulateRibbon(ribbon.Model as IHaveActions);
        ...
        private void HandleViewResultPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            PopulateRibbon(Model as IHaveActions, viewResult.Model as IHaveActions, viewResult.ViewTitle);
        }
    ...
    /// <summary>
    /// Allows for the use of a single Ribbon Page as a stand-alone toolbar
    /// </summary>
    ...
    public class RibbonPageAsToolbar : RibbonPageLayoutPanel
    {
        ...
        private void Populate()
        {
            ...
            if (Actions == null) return;
            var addedRibbonItems = 0;
            foreach (var matchingAction in Actions)
            {
                if (addedRibbonItems > 0 && matchingAction.BeginGroup) Children.Add(new RibbonSeparator());

                if (matchingAction.ActionView != null)
                {
                    if (matchingAction.ActionViewModel != null && matchingAction.ActionView.DataContext == null) matchingAction.ActionView.DataContext = matchingAction.ActionViewModel;
                    var command = ViewActionRibbon.GetRibbonItemCommand(matchingAction.ActionView);
                    if (command == null) ViewActionRibbon.SetRibbonItemCommand(matchingAction.ActionView, matchingAction);
                    ...
                    Children.Add(matchingAction.ActionView);
                }
                else
                {
                    RibbonButton newRibbonButton;
    ...
    public class SpecialFirstPageActionList : Panel
    {
        ...
        private static void SelectedViewChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (actionsContainer != null)
            {
                ...
                viewResult.PropertyChanged -= list.HandleViewResultPropertyChanged;
                viewResult.PropertyChanged += list.HandleViewResultPropertyChanged;
            }
            else
                list.PopulateList(list.Model as IHaveActions);
        ...
        private void HandleViewResultPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            PopulateList(Model as IHaveActions, viewResult.Model as IHaveActions, viewResult.ViewTitle);
        }
