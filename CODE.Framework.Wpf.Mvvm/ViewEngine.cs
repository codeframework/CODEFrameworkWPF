...
namespace CODE.Framework.Wpf.Mvvm
{
    ...
    public class ViewEngineResult
    {
        ...
        /// Constructor
        /// </summary>
        public ViewEngineResult() => LocationsSearched = new List<string>();
    ...
    public class DocumentEngineResult
    {
        ...
        /// Constructor
        /// </summary>
        public DocumentEngineResult() => LocationsSearched = new List<string>();
    ...
    public class LooseXamlViewEngine : IThemableViewEngine, IDocumentEngine
    {
        ...
        {
            if (view == null) return;
            if (view is not View realView) return;
        ...
        protected virtual ViewEngineResult TryGetView(string searchPath)
        {
            try
            {
                ...
                if (Application.LoadComponent(uri) is FrameworkElement view)
                {
                    if (view is IViewInformation viewInfo)
        ...
        {
            if (searchPath.EndsWith(".xaml")) searchPath = searchPath[0..^5];
        ...
        private static bool ApplyResource(FrameworkElement view, string searchPath)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    if (Application.LoadComponent(source) is ResourceDictionary resources)
                    {
        ...
        protected virtual DocumentEngineResult TryGetDocument(string searchPath)
        {
            try
            {
                if (Application.LoadComponent(new Uri(searchPath, UriKind.Relative)) is FrameworkContentElement document)
                {
                    if (document is ISourceInformation documentInfo)
    ...
    public class StandardViewEngine : IThemableViewEngine
    {
        ...
        private static FrameworkElement GetViewInternal(string viewName)
        {
        ...
        public void ChangeViewTheme(FrameworkElement view, string newTheme, string oldTheme)
        {
            if (view is StandardViewGrid grid)
            {
                grid.Children.Clear();
