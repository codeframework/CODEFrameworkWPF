...
using CODE.Framework.Wpf.Theme.Vapor.Standard;

namespace CODE.Framework.Wpf.Theme.Vapor.Classes
{
    /// <summary>
    /// Standard features supported by the Vapor theme
    /// </summary>
    public class VaporStandardFeatures : IThemeStandardFeatures
    {
        /// <summary>Reference to the standard view factory (if supported)</summary>
        public IStandardViewFactory StandardViewFactory => _standardViewFactory;

        private readonly IStandardViewFactory _standardViewFactory = new VaporStandardViewFactory();
    }

    /// <summary>Factory to create standard views supported by Vapor</summary>
    public class VaporStandardViewFactory : IStandardViewFactory
    {
        ...
        public FrameworkElement GetStandardView(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case "LargePeekImageAndText06": return new ListBoxSmartDataTemplate { DefaultColumns = "Image1, Text1" };
        ...
        /// <returns>Standard view or null</returns>
        public FrameworkElement GetStandardView(Mvvm.StandardViews standardView) => standardView == Mvvm.StandardViews.None ? null : GetStandardView(standardView.ToString());
    }
}
