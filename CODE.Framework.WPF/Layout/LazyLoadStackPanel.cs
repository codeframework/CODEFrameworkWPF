    ...
    public class LazyLoadStackPanel : Panel, IScrollInfo
    {
        ...
        private void PopulateInternalChildrenCollection()
        {
            ...
            for (var counter = 0; counter < childCount; counter++)
            {
                ...
                if (currentChild is not ListBoxItem listItem) continue;

                // Get the content
                var contentChild = GetContentPresenterChild(listItem);
                if (contentChild == null) continue;
                if (contentChild is IScrollInfo contentScroll) newItem.ScrollInfo = contentScroll;

                // See if we can observe resizing
                if (contentChild is FrameworkElement contentElement)
                    contentElement.SizeChanged += (s, e) => InvalidateMeasure();

                if (contentChild is IInvalidated invalidated)
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            foreach (var item in _childItems)
            {
                ...
                if (listBoxItem != null && listBoxItem.Content != null && listBoxItem.Content is FrameworkElement)
                {
                    ...
                    // Can this element scroll internally?
                    var elementRect = GetNewRect(currentLeft, currentTop, child is not IHorizontalOffset offsetElement ? (listContentElement.DesiredSize.Width + listBoxItem.Padding.Left + listBoxItem.Padding.Right) : offsetElement.ExtentWidth, child.DesiredSize.Height);
                ...
                if (!specialContentFound)
                {
                    ...
                    // Can this element scroll internally?
                    var elementSize = GetNewRect(currentLeft, currentTop, child is not IHorizontalOffset offsetElement ? child.DesiredSize.Width : offsetElement.ExtentWidth, child.DesiredSize.Height);
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            foreach (var item in _childItems)
            {
                ...
                if (child is ILazyLoad lazyElement && !lazyElement.HasLoaded) lazyElement.Load();
