...
namespace CODE.Framework.Wpf.MarkupExtensions
{
    ...
    public class AttributeBinding : MarkupExtension
    {
        ...
        {
            if (serviceProvider is not IProvideValueTarget valueProvider) return null;
        ...
        public Attribute GetAttribute(object targetObject, object targetProperty)
        {
            ...
            if (source == null)
            {
                if (targetObject is FrameworkElement element)
                {
                    ...
                    if (element.DataContext == null)
                    {
                        ...
                        {
                            if (element.DataContext == null || targetProperty is not DependencyProperty targetProperty2) return;
            ...
            var propertyInfo = ObjectHelper.GetPropertyByPath(source, Path, out var parentObject);
        ...
        public object GetAttributeProperty(object targetObject, object targetProperty)
        {
            ...
            if (source == null)
            {
                if (targetObject is FrameworkElement element)
                {
                    // We do not have a source yet, but if the data context ever changes, we can trigger another update
                    var targetObject2 = (DependencyObject)targetObject;
                    DependencyProperty targetProperty2 = targetProperty as DependencyProperty;
                    if (element.DataContext == null)
                    {
                        element.DataContextChanged += (s, e) =>
                        {
        ...
        {
            if (targetProperty is not DependencyProperty property) return FallbackValue;

            if (property.PropertyType == typeof(int))
            {
                if (FallbackValue is int) return (int)FallbackValue;
                if (int.TryParse(FallbackValue.ToString(), out var outInt)) return outInt;
            }
            if (property.PropertyType == typeof(double))
            {
                if (FallbackValue is double) return (double)FallbackValue;
                if (double.TryParse(FallbackValue.ToString(), out var outDouble)) return outDouble;
            }
            if (property.PropertyType == typeof(decimal))
            {
                if (FallbackValue is decimal) return (decimal)FallbackValue;
                if (decimal.TryParse(FallbackValue.ToString(), out var outDecimal)) return outDecimal;
            }
