...
namespace CODE.Framework.Wpf.Utilities
{
    ...
    public static class ObjectHelper
    {
        ...
        private static TResult GetSimplePropertyValue<TResult>(object valueObject, string propertyName)
        {
            try
            {
                var dynamicObject = valueObject as DynamicObject;
                if (dynamicObject != null)
                {
                    object result = null;
                    if (dynamicObject.TryGetMember(new LocalGetMemberBinder(propertyName), out result))
        ...
        private class LocalGetMemberBinder : GetMemberBinder
        {
            public LocalGetMemberBinder(string name) : base(name, false)
            {
