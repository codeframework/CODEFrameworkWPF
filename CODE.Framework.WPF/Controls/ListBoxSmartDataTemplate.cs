...
namespace CODE.Framework.Wpf.Controls
{
    ...
    public class ListBoxSmartDataTemplate : ColumnPanel
    {
        ...
        public ListBoxSmartDataTemplate()
        {
            ...
            Loaded += (s, a) =>
            {
                ...
                // We save away this template mapping for later use
                var mappings = ListBoxEx.GetListBoxItemToTemplateMappings(listBox);
                if (mappings == null)
                {
                    ListBoxEx.SetListBoxItemToTemplateMappings(listBox, new Dictionary<object, ListBoxSmartDataTemplate>());
                    mappings = ListBoxEx.GetListBoxItemToTemplateMappings(listBox);
                }
                if (mappings.ContainsKey(DataContext))
                    mappings[item] = this;
                else
                    mappings.Add(DataContext, this);
        ...
        /// <summary>
        /// Moves the focus into the defined column. Note that this only really makes sense if there is an editable control in that column.
        /// </summary>
        /// <param name="columnIndex"></param>
        public void MoveFocusToColumn(int columnIndex)
        {
            if (Columns != null && Columns.Count > columnIndex)
            {
                var grid = Children.OfType<Grid>().Where(c => GetColumn(c) == columnIndex).FirstOrDefault();
                if (grid != null)
                {
                    FocusHelper.FocusFirstControlDelayed(grid, 0);
                    return;
                }

                var panel = Children.OfType<Panel>().Where(c => GetColumn(c) == columnIndex).FirstOrDefault();
                if (panel != null)
                {
                    FocusHelper.FocusFirstControlDelayed(panel, 0);
                    return;
                }
            }
        }
        ...
        private void RaiseEditModeChanged() => EditModeChanged?.Invoke(this, new EventArgs());
        ...
        private void HandleColumnCollectionChanged(object sender, NotifyCollectionChangedEventArgs e) => RepopulateColumns(); // This handler is already triggered delayed, so we can go right ahead.
        ...
        private void RepopulateColumns()
        {
            ...
            foreach (var column in Columns.Where(c => c.Visibility == Visibility.Visible))
            {
                    ...
                    switch (column.ColumnControl)
                    {
                        ...
                        case ListColumnControls.Text:
                            ...
                            if (column.EditMode != ListRowEditMode.ReadWriteAll)
                            {
                                ...
                                SetVerticalAlignment(text, column.CellContentVerticalAlignment);
                            ...
                            {
                                tb = new TextBox { HorizontalAlignment = HorizontalAlignment.Stretch };
                                Ex.SetSelectOnEntry(tb, true);
                                SetEventCommands(tb, column.WriteControlEventCommands);
                                SetSpecialKeyPresses(tb);
                                ...
                                SetVerticalAlignment(tb, column.CellContentVerticalAlignment, VerticalAlignment.Stretch);
                                SetVerticalContentAlignment(tb, column.CellContentVerticalAlignment);
                        ...
                        case ListColumnControls.Checkmark:
                            var check = new CheckBox { Content = string.Empty, IsEnabled = false };
                            SetEventCommands(check, column.ReadOnlyControlEventCommands);
                            SetSpecialKeyPresses(check);
                            ...
                            SetVerticalAlignment(check, column.CellContentVerticalAlignment);
                        ...
                        case ListColumnControls.TextList:
                            ...
                            if (column.EditMode != ListRowEditMode.ReadWriteAll)
                            {
                                ...
                                SetVerticalAlignment(text2, column.CellContentVerticalAlignment);
                            ...
                            if (column.EditMode != ListRowEditMode.ReadOnly)
                            {
                                combo = new ComboBox { VerticalAlignment = VerticalAlignment.Stretch, HorizontalAlignment = HorizontalAlignment.Stretch };
                                SetEventCommands(combo, column.WriteControlEventCommands);
                                // SetSpecialKeyPresses(combo); // Not doing this for comboboxes, since they have their own key behavior
                                ...
                                SetVerticalContentAlignment(combo, column.CellContentVerticalAlignment);
                ...
                {
                    var content = GetContentGridForColumn(column);
                    var text = new TextBlock { TextAlignment = TextAlignment.Right };
                    SetHorizontalAlignment(text, column.CellContentAlignment, HorizontalAlignment.Right);
                    SetVerticalAlignment(text, column.CellContentVerticalAlignment);
            ...
            if (Columns.DetailTemplate != null)
                if (Columns.DetailTemplate.LoadContent() is FrameworkElement content)
                {
        ...
        private void SetSpecialKeyPresses(Control control)
        {
            control.PreviewKeyDown += (s, e) =>
            {
                switch (e.Key)
                {
                    case Key.Up:
                        ...
                        if (list != null && ListBoxEx.GetKeyUpDownItemSelectionEnabledInControls(list))
                            if (list.SelectedIndex > 0)
                            {
                                list.SelectedIndex--;
                                SelectCurrentControlInSelectedRow(s as Control, list);
                                e.Handled = true;
                            }
                        break;
                    case Key.Down:
                        var list2 = ElementHelper.FindVisualTreeParent<ListBox>(this);
                        if (list2 != null && ListBoxEx.GetKeyUpDownItemSelectionEnabledInControls(list2))
                            if (list2.SelectedIndex < list2.Items.Count - 1)
                            {
                                list2.SelectedIndex++;
                                SelectCurrentControlInSelectedRow(s as Control, list2);
                                e.Handled = true;
                            }
                        break;
                    case Key.Enter:
                        var list3 = ElementHelper.FindVisualTreeParent<ListBox>(this);
                        if (list3 != null && ListBoxEx.GetSelectNextRowOnEnterInHostedControls(list3))
                            if (list3.SelectedIndex < list3.Items.Count - 1)
                            {
                                list3.SelectedIndex++;
                                SelectCurrentControlInSelectedRow(s as Control, list3);
                                e.Handled = true;
                            }
                        break;
                        // TODO: We could also handle these, but we would have to know how many rows fit without scrolling, which is tricky
                        //case Key.PageUp:
                        //    break;
                        //case Key.PageDown:
                        //    break;
        ...
        private void SelectCurrentControlInSelectedRow(Control control, ListBox list)
        {
            // First, we need to figure out the index of the column this control is in
            var parentGrid = ElementHelper.FindParent<Grid>(control);
            var columnIndex = GetColumn(parentGrid);

            // We can now move the focus 
            ListBoxEx.SetMoveFocusToColumnIndex(list, columnIndex);
        }

        private static void SetVerticalAlignment(FrameworkElement element, ListColumnContentVerticalAlignment desiredAlignment, VerticalAlignment defaultAlignment = VerticalAlignment.Center)
        {
            switch (desiredAlignment)
            {
                case ListColumnContentVerticalAlignment.Top:
                    element.VerticalAlignment = VerticalAlignment.Top;
                    break;
                case ListColumnContentVerticalAlignment.Center:
                    element.VerticalAlignment = VerticalAlignment.Center;
                    break;
                case ListColumnContentVerticalAlignment.Stretch:
                    element.VerticalAlignment = VerticalAlignment.Stretch;
                    break;
                case ListColumnContentVerticalAlignment.Bottom:
                    element.VerticalAlignment = VerticalAlignment.Bottom;
                    break;
                default:
                    element.VerticalAlignment = defaultAlignment;
                    break;
            }
        }

        private static void SetVerticalContentAlignment(Control control, ListColumnContentVerticalAlignment desiredAlignment, VerticalAlignment defaultAlignment = VerticalAlignment.Center)
        {
            switch (desiredAlignment)
            {
                case ListColumnContentVerticalAlignment.Top:
                ...
                case ListColumnContentVerticalAlignment.Bottom:
                    ...
                    break;
                default:
                    control.VerticalContentAlignment = defaultAlignment;
                    break;
            }
        }
    ...
    public class LengthToGridLengthConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (length < 0) return new GridLength();
