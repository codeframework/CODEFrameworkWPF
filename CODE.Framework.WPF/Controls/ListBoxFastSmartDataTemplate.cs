...
namespace CODE.Framework.Wpf.Controls
{
    ...
    public class ListBoxFastSmartDataTemplate : Panel, IScrollInfo, IScrollMeasure
    {
        ...
        {
            if (o is not ListBoxFastSmartDataTemplate item) return;
            if (args.NewValue is not ListColumnsCollection columns)
            {
                item.InvalidateAll();
                return;
            }
        ...
        protected override void OnRender(DrawingContext dc)
        {
            if (ActualWidth > 0 && ActualHeight > 0)
                dc.DrawRectangle(Brushes.Transparent, null, new Rect(0, 0, ActualWidth, ActualHeight)); // This makes sure the entire row is hit-test visible
            ...
            foreach (var column in Columns.Where(c => c.Visibility == Visibility.Visible))
            {
                ...
                {
                    var currentValueObject = DataContext.GetPropertyValue<object>(column.BindingPath);
                    if (currentValueObject != null)
                    {
                        var currentValue = currentValueObject.ToString();

                        var alignment = TextAlignment.Left;
                        switch (column.CellContentAlignment)
                        {
                        ...
                        var top = 0d;
                        if (ft.Height < height - 1)
                            top = (int)((height - ft.Height) / 2);
                        dc.DrawText(ft, new Point(currentLeft, top));
                    }
                }
        ...
        {
            if (Columns == null) return new Size(1, 26);
